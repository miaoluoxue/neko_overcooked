...
class Op:
    ...
    prep: bool = False
    #: ☠ **这条 `cook` 是"回来取菜"那一下**(`Engine._tends`: 台账里那锅快熟了 ⇒ 现在回去).
    #: 只有 `_tends` 会填它。为什么非要一个显式字段: `op_cook` 里有一条**反过来**的闸门
    #: —— "我放进去的那份还没到点 ⇒ 这一步现在不可做, 到点再回来"(`_op_actionable`)。
    #: 两者动作名相同(`cook`)、目标相同, 光看这两个字段**分不开**
    #: (和 `redo`/`prep`/`handoff` 同一个理由: 判据不能靠解析 `note`)。
    tend: bool = False
    ...
    #: **把货源钉死成这一个** —— 递归规划器推出的"必须从这儿拿"(格子 id / 台面 id)。
    #:
    #: 为什么需要它: `_fetch_source_live` 是按**步数最近**挑货源的(那是给贪心层用的)。
    #: 而规划器可能推出一条**非最近但唯一可行**的路 —— 最典型的就是
    #: "这份料只能从**队友背上的背包**拿"(DLC09 背包关)。不钉死的话, 一个更近的箱子
    #: (哪怕是模糊匹配到的假货源)会把规划器的决定**当场覆盖掉**。
    #: ⚠ 空串 = 没钉(老行为: 按距离挑, 一个字不变)。
    pin_src: str = ""
